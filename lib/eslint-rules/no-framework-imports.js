 ...
 *
 * Fix 6: Refactored to use robust path matching utilities for cross-platform
 * compatibility (Windows backslashes, symlinks, case-insensitive filesystems).
 */
const { matchesPattern } = require('./shared/path-utils');
...
const rule = {
  ...
  create(context) {
    ...
    const isFrameworkFile = matchesPattern(filename, '/lib/src/cli/framework/');
    const isCommandFile = matchesPattern(filename, '/lib/src/cli/commands/');
