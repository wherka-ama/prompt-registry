...
>
> **Scope:** all the commands and families of the cli.
---
...
export PR_BIN="node $(pwd)/lib/dist/cli/main.js"
...
```bash
# Simulate VS Code user data directory (Linux: ~/.config/Code/User).
mkdir -p "$PR_TEST_ROOT/xdg/Code/User"
$PR_BIN target add my-vscode \
  --type vscode \
  --path "$PR_TEST_ROOT/xdg/Code/User" \
  -o json
...
$PR_BIN target add my-vscode --type vscode --path "$PR_TEST_ROOT/xdg/Code/User" -o json
```
...
# File should be on disk in the target (VS Code user data dir).
ls "$PR_TEST_ROOT/xdg/Code/User/prompts/"
# expect: a.md
cat "$PR_TEST_ROOT/xdg/Code/User/prompts/a.md"
# expect: "# A prompt"
...
ls "$PR_TEST_ROOT/xdg/Code/User/prompts/" 2>/dev/null
...
# 3. Wipe the user data dir.
rm -rf "$PR_TEST_ROOT/xdg/Code/User"
...
cat "$PR_TEST_ROOT/xdg/Code/User/prompts/a.md"   # "# A prompt"
```
...
for c in "${CMDS[@]}"; do
  for fmt in text json yaml ndjson; do
    ...
    # Allow empty output for ndjson format (empty arrays are valid NDJSON)
    if [ "$fmt" != "ndjson" ]; then
...
## 29. User Scope Installation (GitHub Copilot CLI Compliance)

Tests user scope installation patterns compatible with GitHub Copilot CLI conventions.

### 29.1 User scope target setup

```bash
# Add a user scope target for copilot-cli
$PR_BIN target add copilot-cli-user --type copilot-cli --scope user -o json | jq -e '.data.target.scope == "user"'

# Verify the target was added
$PR_BIN target list -o json | jq -e '.data[] | select(.name=="copilot-cli-user" and .scope=="user")'
```

### 29.2 Install bundle to user scope

```bash
# Install a bundle to user scope (should write to ~/.copilot/prompts/)
$PR_BIN install local-foo --from "$PR_TEST_ROOT/bundles/local-foo" --target copilot-cli-user -o json | jq -e '.status == "ok"'

# Verify files were written to user scope location
# prompt-registry uses ${HOME}/.copilot as the default base for copilot-cli
# targets, matching the GitHub Copilot CLI convention for user-level files.
USER_DIR="$HOME/.copilot"
test -d "$USER_DIR"
test -f "$USER_DIR/prompts/a.md"
```

### 29.3 User scope search

```bash
# Search for primitives in user scope
$PR_BIN index search --q "hello" --index "$PR_TEST_ROOT/idx-corpus/idx.json" -o json | jq '.data.total'
# expect: >= 0
```

---

## 30. Repository Scope Installation (GitHub Copilot CLI Compliance)

Tests repository scope installation patterns for the `vscode` repository scope.
VS Code Copilot reads from `.github/` subdirectories: `prompts/` → `.github/prompts/`,
`instructions/` → `.github/instructions/`, `chatmodes/` → `.github/chatmodes/`,
`skills/` → `.github/skills/` (Agent Skills standard — SKILL.md + resources).

### 30.1 Repository scope target setup

```bash
# Create a git repository for testing
mkdir -p "$PR_TEST_ROOT/repo-test"
cd "$PR_TEST_ROOT/repo-test"
git init

# Add a repository scope target
$PR_BIN target add repo-vscode --type vscode --scope repository --workspaceRoot "$PR_TEST_ROOT/repo-test" -o json | jq -e '.data.target.scope == "repository"'
```

### 30.2 Install bundle to repository scope

```bash
# Activate a profile that installs to repository scope
$PR_BIN profile activate backend -o json | jq -e '.status == "ok"'

# Verify files were written to .github/ subdirectories
# (vscode repository scope: prompts/ → .github/prompts/)
test -d "$PR_TEST_ROOT/repo-test/.github/prompts"
test -f "$PR_TEST_ROOT/repo-test/.github/prompts/a.md"

# Verify skills directory structure (Agent Skills standard)
# skills/ bundle content routes to .github/skills/ for vscode repository scope.
# agents/ is NOT routed for vscode — Copilot agents are plugin-distributed.
if [ -d "$PR_TEST_ROOT/repo-test/.github/skills" ]; then
  test -n "$(ls -A "$PR_TEST_ROOT/repo-test/.github/skills")"
fi
```

### 30.3 Repository scope with commit mode

```bash
# Add repository target with commit mode
$PR_BIN target add repo-commit --type vscode --scope repository --workspaceRoot "$PR_TEST_ROOT/repo-test" --commit-mode local-only -o json | jq -e '.data.target.commitMode == "local-only"'

# Verify .git/info/exclude is updated
test -f "$PR_TEST_ROOT/repo-test/.git/info/exclude"
grep -q "Prompt Registry (local)" "$PR_TEST_ROOT/repo-test/.git/info/exclude"
```

---

## 31. Uninstallation Tests

Tests uninstallation of bundles from user and repository scopes.

The uninstall command supports three modes:
- By bundle ID: `prompt-registry uninstall <bundle-id> --target <name>`
- From lockfile: `prompt-registry uninstall --lockfile <path>`
- All bundles: `prompt-registry uninstall --all --target <name>`

### 31.1 Uninstall by bundle ID

```bash
# Activate a profile first to have installed bundles
$PR_BIN profile activate backend -o json | jq -e '.status == "ok"'

# Uninstall a specific bundle by ID
$PR_BIN uninstall local-foo --target my-vscode -o json | jq -e '.status == "ok"'

# Verify files were removed from target
test ! -f "$PR_TEST_ROOT/xdg/Code/User/prompts/a.md" || echo "File should be removed"

# Verify lockfile entry was removed
cat "$PR_TEST_ROOT/project/prompt-registry.lock.json" | jq '.entries[] | select(.bundleId == "local-foo")'
# expect: empty (no entry)
```

### 31.2 Uninstall from lockfile

```bash
# Re-install to have a lockfile with entries
$PR_BIN profile activate backend -o json | jq -e '.status == "ok"'

# Uninstall all bundles from lockfile
$PR_BIN uninstall --lockfile "$PR_TEST_ROOT/project/prompt-registry.lock.json" --target my-vscode -o json | jq -e '.status == "ok"'

# Verify all files were removed
test ! -f "$PR_TEST_ROOT/xdg/Code/User/prompts/a.md" || echo "File should be removed"
```

### 31.3 Uninstall all bundles for target

```bash
# Re-install to have bundles
$PR_BIN profile activate backend -o json | jq -e '.status == "ok"'

# Uninstall all bundles for a target
$PR_BIN uninstall --all --target my-vscode -o json | jq -e '.status == "ok"'

# Verify all files were removed
test ! -f "$PR_TEST_ROOT/xdg/Code/User/prompts/a.md" || echo "File should be removed"
```

### 31.4 Uninstall error handling

```bash
# Uninstall non-existent bundle
$PR_BIN uninstall nonexistent-bundle --target my-vscode -o json | jq -e '.status == "warning"'

# Uninstall without --target when no previous target exists
$PR_BIN target remove my-vscode
$PR_BIN uninstall local-foo -o json | jq -e '.errors[0].code == "USAGE.MISSING_FLAG"'
...
## 32. Primitive Search Tests

Tests search functionality for different primitive kinds (prompt, skill, agent, instruction, mcp-server).

### 32.1 Search by kind

```bash
# Search for prompts only
$PR_BIN index search --index "$PR_TEST_ROOT/idx-corpus/idx.json" -q "hello" --kinds prompt -o json | jq -e 'all(.data.hits[].primitive.kind == "prompt")'

# Search for skills only
$PR_BIN index search --index "$PR_TEST_ROOT/idx-corpus/idx.json" -q "test" --kinds skill -o json | jq '.data.total'
# expect: >= 0
```

### 32.2 Search by source

```bash
# Search from local source only
$PR_BIN index search --index "$PR_TEST_ROOT/idx-corpus/idx.json" -q "hello" --sources local -o json | jq -e 'all(.data.hits[].primitive.bundle.sourceId == "local")'
```

### 32.3 Search by bundle

```bash
# Search within specific bundle
$PR_BIN index search --index "$PR_TEST_ROOT/idx-corpus/idx.json" -q "hello" --bundles alpha -o json | jq -e 'all(.data.hits[].primitive.bundle.bundleId == "alpha")'
```

### 32.4 Search with limit and offset (pagination)

```bash
# First page
$PR_BIN index search --index "$PR_TEST_ROOT/idx-corpus/idx.json" -q "hello" --limit 1 --offset 0 -o json | jq '.data.hits | length'
# expect: 1

# Second page
$PR_BIN index search --index "$PR_TEST_ROOT/idx-corpus/idx.json" -q "hello" --limit 1 --offset 1 -o json | jq '.data.hits | length'
# expect: 1
```

---

## 33. GitHub Copilot CLI Directory Conventions

Verify that prompt-registry's directory structure matches GitHub Copilot CLI conventions.

### 33.1 Skills directory structure

```bash
# GitHub Copilot CLI expects .github/skills/<skill-name>/SKILL.md
# Verify repository scope writer uses this structure
if [ -d "$PR_TEST_ROOT/repo-test/.github/skills" ]; then
  for skill_dir in "$PR_TEST_ROOT/repo-test/.github/skills"/*; do
    test -f "$skill_dir/SKILL.md" || echo "Expected SKILL.md in $skill_dir"
  done
fi
```

### 33.2 Alternative skill directories

```bash
# GitHub Copilot CLI also supports .claude/skills and .agents/skills
# Verify these are handled correctly
for alt_dir in ".claude/skills" ".agents/skills"; do
  if [ -d "$PR_TEST_ROOT/repo-test/$alt_dir" ]; then
    echo "Alternative skill directory $alt_dir detected"
  fi
done
```
...
## 34. Cross-Scope Installation

Tests installation of the same bundle to both user and repository scopes.

### 34.1 Install to both scopes

```bash
# Install to user scope
$PR_BIN install local-foo --from "$PR_TEST_ROOT/bundles/local-foo" --target copilot-cli-user -o json | jq -e '.status == "ok"'

# Install to repository scope
$PR_BIN install local-foo --from "$PR_TEST_ROOT/bundles/local-foo" --target repo-vscode -o json | jq -e '.status == "ok"'

# Verify both installations exist
# copilot-cli user scope default: ~/.copilot/prompts/
USER_DIR="$HOME/.copilot"
test -f "$USER_DIR/prompts/a.md"
# vscode repository scope: .github/prompts/ (not .github/copilot/prompts/)
test -f "$PR_TEST_ROOT/repo-test/.github/prompts/a.md"
```

### 34.2 Uninstall from one scope

```bash
# Uninstall from user scope only
# Future: $PR_BIN uninstall local-foo --target copilot-cli-user -o json | jq -e '.status == "ok"'

# Verify user scope removed but repository scope still exists
# USER_DIR="$HOME/.copilot"
# test ! -f "$USER_DIR/prompts/a.md" || echo "User file should be removed"
# test -f "$PR_TEST_ROOT/repo-test/.github/prompts/a.md"
```

---

## 35. Sign-off Checklist (Extended)

Additional sign-off items for user/repo scope and GitHub Copilot CLI compliance.

- [ ] User scope installation works correctly
- [ ] Repository scope installation works correctly
- [ ] Files placed in correct directories (`.github/prompts/`, `.github/skills/`, `~/.copilot/prompts/`, etc.)
- [ ] Skills follow `SKILL.md` naming convention
- [ ] Uninstallation removes files from correct scope
- [ ] Lockfile correctly tracks scope-specific installations
- [ ] Search filters work correctly by kind, source, bundle
- [ ] Pagination (limit/offset) works correctly
- [ ] Cross-scope installation allows independent management
- [ ] Commit mode (local-only vs commit) works correctly
- [ ] .git/info/exclude updated correctly for local-only mode
