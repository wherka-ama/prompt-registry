...
function detectAffected({ repoRoot, changedPaths, spawnSync }) {
  ...
  // Phase 4: detect-affected-collections shim wraps the response in
  // the new envelope shape ({schemaVersion, data: {affected}}). Accept
  // both shapes for backward-compat with the legacy bare {affected}.
  return (parsed.data && parsed.data.affected) || parsed.affected || [];
}
...
function computeVersion({ repoRoot, collectionFile, spawnSync }) {
  ...
  const parsed = JSON.parse(out);
  // Phase 4: compute-collection-version shim wraps in {data: {...}}.
...
function buildBundle({ repoRoot, repoSlug, collectionFile, version, spawnSync }) {
  ...
  const parsed = JSON.parse(out);
  // Phase 4: bundle.build shim wraps the response in {data: {...}}.
  return parsed.data || parsed;
}
