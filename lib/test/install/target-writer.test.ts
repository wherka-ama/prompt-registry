...
describe('Phase 5 / Iter 18-19 — FileTreeTargetWriter', () => {
  ...
  it('windsurf maps prompts/instructions -> rules, skills -> skills, no agents route', () => {
    ...
    assert.strictEqual(layout.kindRoutes['skills/'], 'skills/');
    assert.strictEqual(layout.kindRoutes['agents/'], undefined);
  });
  ...
  it('windsurf collapses prompts and instructions into rules/, skips unrouted agents/', async () => {
    ...
    assert.deepStrictEqual(result.skipped, ['agents/a.md']);
  });
  ...
  it('kiro user scope routes prompts/instructions to steering/ and agents to agents/', () => {
    ...
    assert.strictEqual(layout.kindRoutes['prompts/'], 'steering/');
    assert.strictEqual(layout.kindRoutes['instructions/'], 'steering/');
    assert.strictEqual(layout.kindRoutes['agents/'], 'agents/');
    assert.strictEqual(layout.kindRoutes['chatmodes/'], undefined);
  });
  ...
  it('kiro repository scope routes into .kiro/ subdirs', () => {
    ...
    assert.strictEqual(layout.baseDir, '/repo');
    assert.strictEqual(layout.kindRoutes['prompts/'], '.kiro/steering/');
    assert.strictEqual(layout.kindRoutes['agents/'], '.kiro/agents/');
    assert.strictEqual(layout.kindRoutes['instructions/'], '.kiro/steering/');
  });
  ...
  it('copilot-cli default base dir is ~/.copilot; skills routed, agents not (plugin-distributed)', () => {
    ...
    assert.strictEqual(layout.baseDir, '${HOME}/.copilot');
    assert.strictEqual(layout.kindRoutes['prompts/'], 'prompts/');
    assert.strictEqual(layout.kindRoutes['skills/'], 'skills/');
    assert.strictEqual(layout.kindRoutes['agents/'], undefined);
  });
  ...
  it('vscode repository scope: skills go to .github/skills/, agents not routed', () => {
    ...
    assert.strictEqual(layout.baseDir, '/repo');
    assert.strictEqual(layout.kindRoutes['prompts/'], '.github/prompts/');
    assert.strictEqual(layout.kindRoutes['instructions/'], '.github/instructions/');
    assert.strictEqual(layout.kindRoutes['skills/'], '.github/skills/');
    assert.strictEqual(layout.kindRoutes['agents/'], undefined);
  });
  ...
  it('windsurf repository scope: rules and skills routed, agents not (no native agent concept)', () => {
    ...
    assert.strictEqual(layout.baseDir, '/repo');
    assert.strictEqual(layout.kindRoutes['prompts/'], '.windsurf/rules/');
    assert.strictEqual(layout.kindRoutes['skills/'], '.windsurf/skills/');
    assert.strictEqual(layout.kindRoutes['agents/'], undefined);
  });
  ...
  it('claude-code repository scope routes skills to .claude/skills/ and agents to .claude/agents/', () => {
    ...
    assert.strictEqual(layout.baseDir, '/repo');
    assert.strictEqual(layout.kindRoutes['prompts/'], '.claude/commands/');
    assert.strictEqual(layout.kindRoutes['agents/'], '.claude/agents/');
    assert.strictEqual(layout.kindRoutes['skills/'], '.claude/skills/');
  });
});
