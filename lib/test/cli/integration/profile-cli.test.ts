...
`);
  ...
  it('switching profiles removes files from the previous profile', () => {
    const env = { XDG_CONFIG_HOME: xdg };
    // Set up a second bundle for the "writer" profile.
    const writerBundleDir = path.join(work, 'bundles', 'bar');
    fs.mkdirSync(path.join(writerBundleDir, 'prompts'), { recursive: true });
    fs.writeFileSync(
      path.join(writerBundleDir, 'deployment-manifest.yml'),
      'id: bar\nversion: 1.0.0\nname: Bar\n'
    );
    fs.writeFileSync(path.join(writerBundleDir, 'prompts', 'b.md'), 'B');

    // Rewrite hub config to include two profiles with distinct bundles.
    fs.writeFileSync(path.join(hubDir, 'hub-config.yml'), `version: 1.0.0
metadata:
  name: My Hub
  description: hub
...
`);
    ...
    // Add hub and target.
    run(['hub', 'add', '--type', 'local', '--location', hubDir, '-o', 'json'], { cwd: work, env });
    run(['target', 'add', 'my-vscode', '--type', 'vscode', '--path', vscodeDir, '-o', 'json'], { cwd: work, env });

    // Activate backend profile → a.md should exist.
    const act1 = run(['profile', 'activate', 'backend', '-o', 'json'], { cwd: work, env });
    assert.strictEqual(act1.code, 0, `backend activate stderr=${act1.stderr}; stdout=${act1.stdout}`);
    assert.ok(fs.existsSync(path.join(vscodeDir, 'prompts', 'a.md')), 'a.md should exist after backend activation');

    // Switch to writer profile → a.md must be gone, b.md must exist.
    const act2 = run(['profile', 'activate', 'writer', '-o', 'json'], { cwd: work, env });
    assert.strictEqual(act2.code, 0, `writer activate stderr=${act2.stderr}; stdout=${act2.stdout}`);
    assert.ok(!fs.existsSync(path.join(vscodeDir, 'prompts', 'a.md')), 'a.md must be removed after switching to writer');
    assert.ok(fs.existsSync(path.join(vscodeDir, 'prompts', 'b.md')), 'b.md should exist after writer activation');
  });
});
